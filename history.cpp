...
void History::load(){
    IO::geti()->log("Odczytywanie historii poleceń...");
    history = get_nonempty_lines(Config::geti()->history_filename);
}

bool History::save(){
    IO::geti()->log("Zapisywanie historii poleceń...");
    stringstream ss;
...
void History::truncate(unsigned int remains){
    stringstream ss;
    ss<<"Usuwanie starej historii poleceń (liczba poleceń: "<<history->size()<<", ograniczenie: "<<remains<<")";
    IO::geti()->log(ss.str());
