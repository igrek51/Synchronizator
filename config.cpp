...
void Config::load_config(){
    if(!file_exists(config_filename)){
        IO::geti()->message_box("Błąd", "Brak pliku konfiguracyjnego - ładowanie wartości domyślnych");
        return;
    }
    IO::geti()->log("Wczytywanie ustawień z pliku konfiguracyjnego...");
    vector<ConfigVariable*>* variables = get_config_variables(config_filename);
    //odczyt zmiennych
    save_wnd_pos = get_config_int(variables, "save_wnd_pos", 0);
    if(save_wnd_pos == 2){ //reset ustawień
...
void Config::add_variable_line(vector<string>* lines, ConfigVariable* variable){
    //szukanie istniejącej linii
    for(unsigned int i=0; i<lines->size(); i++){
        if(string_begins(lines->at(i), variable->name)){
            //modyfikacja już istniejacej linii
